...
class CSFCustomSliderApplicationService(CommonService, HasLog):
    ...
    def _get_facial_attributes(self, sim_info: SimInfo) -> PersistenceBlobs_pb2.BlobSimFacialCustomizationData:
    ...
    def _set_facial_attributes(self, sim_info: SimInfo, new_facial_attributes: PersistenceBlobs_pb2.BlobSimFacialCustomizationData):
        sim_info.facial_attributes = new_facial_attributes.SerializeToString()
        sim_info.resend_facial_attributes()

    def _get_existing_modifiers_for_edit(self, sim_info: SimInfo, exclude_modifier_ids: Iterator[int]=()) -> PersistenceBlobs_pb2.BlobSimFacialCustomizationData:
        facial_attributes = self._get_facial_attributes(sim_info)
        existing_modifiers = PersistenceBlobs_pb2.BlobSimFacialCustomizationData()
    ...
    def get_current_facial_modifier_value(self, sim_info: SimInfo, custom_slider: CSFCustomSlider) -> float:
        ...
        for face_modifier in facial_attributes.face_modifiers:
            ...
            if custom_slider.has_negative_modifier_id() and face_modifier.key == custom_slider.negative_modifier_id:
                self.log.debug('Found existing negative modifier value {} with key {}'.format(face_modifier.amount, face_modifier.key))
            ...
            if custom_slider.has_positive_modifier_id() and face_modifier.key == custom_slider.positive_modifier_id:
                self.log.debug('Found existing positive modifier value {} with key {}'.format(face_modifier.amount, face_modifier.key))
    ...
    def apply_slider(self, sim_info: SimInfo, custom_slider: CSFCustomSlider, amount: float) -> bool:
        ...
        try:
            ...
            modified_facial_attributes = self._get_existing_modifiers_for_edit(sim_info, exclude_modifier_ids=(*custom_slider.get_modifier_ids(), slider_id))

            if slider_amount != 0.0:
                new_modifier = PersistenceBlobs_pb2.BlobSimFacialCustomizationData().Modifier()
